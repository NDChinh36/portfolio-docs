...
```
[Insert: QR Payment confirmation screen annotated]
Bao gồm: merchant name + logo, amount, transaction reference,
wallet balance check indicator, authentication method selector
(PIN primary / Biometric shortcut), và idempotency key generation
flow chú thích rõ tại bước nào key được tạo và validated.
```

```
[Insert: Transaction state machine diagram — 15 states]
Full diagram với tất cả transitions, bao gồm:
- Timeout → Unknown path (highlight màu khác)
- Mismatched → Refund Required / Manually Resolved paths
- Actor trigger annotation trên mỗi transition (who triggers what)
- "Money state" annotation (hold / confirmed / released / pending refund)
```

```
[Insert: Reconciliation dashboard wireframe]
Bao gồm: mismatch queue với severity tags (Critical / High / Normal),
filter bar (date range, status, amount range, merchant),
transaction detail panel (ví record vs bank record side-by-side),
manual resolve action dropdown + required reason field,
audit trail log hiển thị mọi action với actor và timestamp.
```

```
[Insert: Exception handling decision tree]
Bao gồm: timeout detection → Unknown state logic,
retry policy (when to retry, max attempts, backoff),
idempotency check flow (lock → check → write order),
callback failure handling (late ACK sau timeout),
và escalation path khi exception không resolve trong SLA.
```
