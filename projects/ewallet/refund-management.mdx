...
|---|---|---|---|---|
| **BR-REFUND-01** | Refund request được tạo | Amount < 500,000 VND **và** chưa vượt ngưỡng velocity: tối đa **3 auto-refund/user/ngày**, tổng cộng ≤ **1,000,000 VND/user/ngày** | Hệ thống tự động chuyển sang `AUTO_APPROVED` → `EXECUTING` không cần phê duyệt thủ công. Nếu vượt ngưỡng velocity (dù amount < 500K) → chuyển sang `PENDING_APPROVAL`. Velocity check nhằm ngăn tấn công inject event tự động | Refund Service |
